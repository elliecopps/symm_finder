...
def main():
    ...
    parser.add_argument('-seed_range', type=int,
                        help='Number of seeds to test')
    parser.add_argument('-max_energy', type=int,
                        help='Maximum energy level to search for symmetries in')
    ...
    for seed in range(seed, (seed+seed_range)):
        ...
        transformations = sym_mod.list_transformations(
            grouped_configurations, max_energy, N, basis)

        symmetries = sym_mod.symmetry(
            transformations[0], grouped_configurations, basis)
        ...
        degen = (sym_mod.degen_sym(
            transformations[1], grouped_configurations[0], classified_sym))
